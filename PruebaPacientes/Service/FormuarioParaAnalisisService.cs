...
namespace Service
{
    public class FormuarioParaAnalisisService
    {
        ...
        public void UpdateRange(List<FormularioParaAnalisis> FPA)
        {
            dataContext.ListaFormularioParaAnalisis.UpdateRange(FPA);
            dataContext.SaveChanges();
        }
        ...
        public void AddRange(List<FormularioParaAnalisis> ListaDeFPA)
        {
            dataContext.ListaFormularioParaAnalisis.AddRange(ListaDeFPA);
            dataContext.SaveChanges();
        }
        ...
        public List<FormularioParaAnalisis> GetFormulariosGenerados()
        {
            var e  = (from a in dataContext.ListaFormularioParaAnalisis
                                       where a.EstadoDelFormulario == EstadoDelFormulario.GENERADOS
                                       select a).Include("AntecedenteEpidemiologicos1")
                .Include("AntecedenteEpidemiologicos2")
                .Include("Paciente")
                .Include("SignosYSintomas")
                .Include("EnfermedadesPreviasComorbilidades").ToList();

            return e;
        }

        public List<FormularioParaAnalisis> MarcarFormulario(List<FormularioParaAnalisis> formulario, EstadoDelFormulario estado)
        {
            foreach (var item in formulario)
            {
                item.EstadoDelFormulario = estado;
            }
            return formulario;
        }
        ...
        public void SetEnviado(int FormularioId)
        {
            var formulario = dataContext.ListaFormularioParaAnalisis
                .Where(f => f.Id == FormularioId)
                .SingleOrDefault();

            if (formulario != null)
            {
                formulario.EstadoDelFormulario = EstadoDelFormulario.ENVIADO;
                dataContext.ListaFormularioParaAnalisis.Update(formulario);
                dataContext.SaveChanges();
            }
            else
            {
